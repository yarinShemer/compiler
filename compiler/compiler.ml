...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec macro_expand_qq = function
    ...
    |ScmPair (ScmPair (ScmSymbol "unquote-splicing", ScmPair (sexpr, ScmNil)),ScmNil) ->
       sexpr
    ...
    |ScmVector sexpr -> 
      if(list_contains_unquote_splicing sexpr)
          then let vector_to_list = List.fold_right (fun car cdr -> ScmPair (car, cdr)) sexpr ScmNil in
               let expanded_sexpr = macro_expand_qq vector_to_list in
               ScmPair(ScmSymbol "list->vector", ScmPair(expanded_sexpr,ScmNil))
      else let map_on_list = List.map macro_expand_qq sexpr in
           let format_pairs = List.fold_right (fun car cdr -> ScmPair (car, cdr)) map_on_list ScmNil in
           ScmPair(ScmSymbol "vector", format_pairs) 
    |ScmPair(car,cdr) ->
      (match car with
      |ScmPair(ScmSymbol "unquote-splicing", ScmPair (sexpr, ScmNil)) ->
        let expanded_cdr = macro_expand_qq cdr in
        ScmPair(ScmSymbol "append", ScmPair(sexpr, ScmPair(expanded_cdr, ScmNil)))
      | _ ->
  ...
  let rec macro_expand_cond_ribs = function 
      | ScmNil -> ScmVoid
      | ScmPair(ScmPair(ScmSymbol "else", else_exps), ribs) ->
        ScmPair(ScmSymbol "begin", else_exps)
                  ...
                  (ScmPair
                     (ScmSymbol "rest",
                      ScmPair
                        (ScmPair
                           (ScmSymbol "lambda",
                            ScmPair (ScmNil, ScmPair (macro_expand_cond_ribs ribs, ScmNil))),
                         ScmNil)),
                  ScmNil))),
             ScmPair
              (ScmPair
      ...
      | ScmPair (ScmPair (pred, es), ribs) ->
         let then_expr = ScmPair (ScmSymbol "begin", es) in
         let else_expr = macro_expand_cond_ribs ribs in
         ScmPair (ScmSymbol "if",
                  ScmPair (pred, ScmPair (then_expr, ScmPair (else_expr, ScmNil))))
      | _ -> raise(X_syntax "Not a right cond struct");;
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address =
    let rec run expr params env =
      ...
      | ScmApplic (proc, args) ->
        ScmApplic'(run proc params env, List.map (fun arg -> run arg params env) args , Non_Tail_Call)
    in
    fun expr -> run expr [] [];;
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmApplic' (proc, args, Tail_Call) -> 
         let recycle_frame_loop = make_tc_applic_recycle_frame_loop() in
         let applic_recycle_frame_done = make_tc_applic_recycle_frame_done() in
         (String.concat "" (List.map (fun arg -> (run params env arg) ^ "\tpush rax\n") (List.rev args)))
         ^ (Printf.sprintf "\tpush %d\n" (List.length args))
         ^ run params env proc
         ^  "\tcmp byte [rax], T_closure \n"
         ...
         ^ "\t mov rcx, 0\n"                          (*counter*)
         ^ (Printf.sprintf "%s: \n" (recycle_frame_loop))
         ^ (Printf.sprintf "\t cmp rcx, %d +4 \n" (List.length args))
         ^ (Printf.sprintf "\t je %s\n" (applic_recycle_frame_done))
         ^"\t mov r10,[r9] \n"
         ^"\t mov [r8],r10 \n"
         ^"\t inc rcx \n"
         ^"\t sub r8, 8 \n"
