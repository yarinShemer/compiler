...
L_code_ptr_bin_apply:
        ...
        mov rax, PARAM(0)
        assert_closure(rax) 
        mov rdx, PARAM(1)                            ;list s1
        mov rbx, rdx
        mov rcx, 0                                   ; counter- length of list
loop_length:
        cmp byte [rbx], T_nil
        je end_loop_length
        assert_pair(rbx)
...
loop_fill_stack:
        cmp byte [rdx] ,T_nil
        je end_loop_fill_stack
        mov r10, SOB_PAIR_CAR(rdx)
        mov [rsp + 8*rdi], r10
...
end_loop_fill_stack:
        push rcx
        push SOB_CLOSURE_ENV(rax)                  ;env of f
        push qword [rbp + 8*1]
        mov r15, SOB_CLOSURE_CODE(rax)
        mov r14, qword [rbp]
        mov rbx, rcx                               
        add rcx, 3                                ;counter
        lea rsi, [rsp + 8*(2 +rbx)]
